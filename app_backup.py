...
def initialize_gemini_llm():
    """Initialize Gemini LLM - optimized for complete responses"""
    try:
        return ChatGoogleGenerativeAI(
            ...
            timeout=20.0,  # 20s timeout for complete responses
            max_retries=1,
        )
    except Exception as e:
...
def initialize_perplexity_llm():
    """Initialize Perplexity LLM as fallback - optimized for complete responses"""
    try:
...
""")
        ...
        # Create agent executor optimized for complete responses
        agent_executor = AgentExecutor(
            agent=agent,
            tools=tools,
            verbose=False,  # Reduced logging for cleaner output
            handle_parsing_errors=True,
            max_iterations=15,  # Increased iterations to ensure complete responses (no truncation)
            early_stopping_method="force",  # Stop early when answer is complete
            max_execution_time=25.0  # Allow up to 25s for complete responses
        )
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.info("✓ Training prompt loaded")
        
        # Only initialize vectorstore (no tokens consumed if it already exists)
        vectorstore = initialize_vectorstore(RESUME_CONTENT)
        logger.info("✓ Vectorstore initialized (text-only, no embeddings)")
        
        # LLM and agent will be lazy-loaded on first request
        logger.info("✓ Lazy initialization enabled (LLMs load on first request)")
        logger.info("="*80)
        logger.info("✅ AI AGENT BACKEND READY - Zero tokens consumed at startup!")
        logger.info("="*80)
    except Exception as e:
...
async def ask_question(request: QuestionRequest):
    ...
    try:
        ...
        if cached_response:
            ...
            logger.info(f"Cached answer preview: {cached_response['answer'][:100]}...")
        ...
        # Get answer from AI agent with timeout for complete responses
        result = None
        max_retries = 2  # Allow retries for robustness
        timeout_seconds = 30.0  # Allow 30s for complete responses (no truncation)
        ...
        for attempt in range(max_retries):
            try:
                ...
                try:
                    ...
                    logger.info(f"✓ Agent execution successful (within {timeout_seconds}s timeout)")
                    break  # Success, exit retry loop
                except asyncio.TimeoutError:
                    logger.error("="*80)
                    logger.error(f"⏱️  TIMEOUT after {timeout_seconds}s - Agent execution exceeded time limit")
                    logger.error(f"Question: {request.question}")
                    logger.error("="*80)
                    # Timeout - let it retry or fail
                    if attempt < max_retries - 1:
                        continue
                    else:
                        raise HTTPException(
                            status_code=status.HTTP_504_GATEWAY_TIMEOUT,
                            detail="Request took too long to process. Please try again with a simpler question."
                        )
                ...
                if should_use_fallback_llm(e):
                    logger.warning("="*80)
                    logger.warning(f"🔄 API ERROR - Attempting fallback to Perplexity")
                    logger.warning(f"Error type: {type(e).__name__}")
                    logger.warning(f"Error message: {str(e)[:100]}")
                    ...
                    if current_llm_provider == "gemini":
                        if fallback_llm or switch_to_fallback_llm():
                            ...
                            logger.error("Cannot initialize fallback LLM - no API key available")
                            if attempt < max_retries - 1:
                                continue
                            else:
                                raise HTTPException(
                    ...
                    logger.error(f"Non-API error: {str(e)[:100]}")
                    if attempt < max_retries - 1:
                        continue
                    else:
                        raise HTTPException(
                            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                            detail=f"An error occurred while processing your question."
                        )
        ...
        if "intermediate_steps" in result:
            logger.info(f"Checking intermediate steps: {len(result['intermediate_steps'])} steps found")
            for idx, step in enumerate(result["intermediate_steps"]):
                if len(step) >= 2:  # Check if there's an observation
                    ...
                    logger.info(f"Step {idx+1}: Tool used = {action_name}")
                    if "ResumeSearch" in action_name or "resume" in action_name.lower():
                        used_resume = True
                        logger.info(f"✓ ResumeSearch tool was used!")
                    if step[1]:
                        source_text = step[1][:100] + "..." if len(step[1]) > 100 else step[1]
                        sources.append(source_text)
        else:
            logger.info("No intermediate_steps in result - agent may have given direct answer")
        
        logger.info(f"Final used_resume value: {used_resume}")
        logger.info(f"is_resume_question: {is_resume_question}")
        ...
        logger.info(f"Used resume data: {used_resume}")
        logger.info(f"Resume question detected: {is_resume_question}")
        logger.info(f"Sources used: {len(sources)}")
        logger.info(f"LLM Provider: {current_llm_provider}")
        logger.info("-"*80)
